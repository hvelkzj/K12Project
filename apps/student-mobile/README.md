...
最终课程测试 APK 为 `K12学习空间-Android-0.1.2.apk`，内置版本号 `102` 和 Camera 原生模块。安装包 SHA-256 为 `482f70027dbcc7d4f7bc3b9f053064a9b973109caa40fc434496f3c43c010099`。APK 作为交付产物单独保存，不提交到 Git 仓库。

## 时间与拍照

- APP 与小程序中的作业截止、考勤和课件日期统一按中国标准时间（UTC+8）显示中文年月日，不依赖设备语言。
- 作业详情分别提供“拍照”和“选择图片”。拍照只打开相机并生成一张图片，相册最多选择三张图片。
- Android 测试包已在 `manifest.json` 启用 Camera 原生模块，并声明相机权限；首次拍照时按系统提示授权。
- 用户取消或拒绝权限时保留作业正文，并显示中文处理提示，不会误报提交成功。
