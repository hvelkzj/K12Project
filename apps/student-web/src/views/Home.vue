...
const lastSubmitted = ref<Submission | null>(null)
...
function openAssignment(id: number): void { selectedId.value = id; lastSubmitted.value = null; errorMessage.value = ''; page.value = 'detail' }
...
async function handleFileChange(event: Event): Promise<void> {
  ...
  errorMessage.value = ''
  try {
    const uploaded = [] as FileSummary[]
    for (const file of files) {
      uploaded.push(
        await studentBusinessClient.uploadFile(
          file,
          normalizeAttachmentMimeType(file.name, file.type),
        ),
      )
    }
...
async function submit(): Promise<void> {
  ...
  try {
    ...
    lastSubmitted.value = submission
...
<template>
  ...
  <section v-else-if="assignment" class="assignment-workspace">
    ...
    <article v-if="page === 'detail'" class="panel">
      <div class="detail-hero"><div><p class="eyebrow">我的作业 · 作业 #{{ assignment.id }}</p><h1>{{ assignment.title }}</h1><p class="description">{{ assignment.description }}</p></div><span :class="['status-pill', isClosed ? 'closed' : 'open']">{{ isClosed ? '已截止' : '可提交' }}</span></div>
      <dl class="meta"><div><dt>◷ 截止时间</dt><dd>{{ formatDateTime(assignment.dueAt) }}</dd></div><div><dt>↗ 提交规则</dt><dd>{{ assignment.allowLate ? '允许迟交' : '截止后不可提交' }}</dd></div><div><dt>✓ 当前状态</dt><dd>{{ isClosed && selectedRow?.status === 'NOT_SUBMITTED' ? '已截止' : selectedRow ? getSubmissionStatusLabel(selectedRow.status) : '未提交' }}</dd></div></dl>
      <div v-if="assignment.attachments.length" class="attachments"><div class="section-title"><span>附件资料</span><small>{{ assignment.attachments.length }} 个文件</small></div><div v-for="file in assignment.attachments" :key="file.id" class="attachment-row"><span class="file-icon">{{ file.originalName.split('.').at(-1)?.toUpperCase() ?? '文件' }}</span><strong>{{ file.originalName }}</strong><small>{{ Math.ceil(file.byteSize / 1024) }} KB</small><button class="download-button" type="button" :disabled="downloadingFileId !== null" @click="downloadAttachment(file)">{{ downloadingFileId === file.id ? '下载中…' : '下载' }}</button></div></div>
    ...
    <form v-else-if="page === 'submit'" class="panel" @submit.prevent="submit">
      ...
      <label class="upload-zone"><input type="file" multiple accept=".pdf,.docx,.jpg,.jpeg,.png" :disabled="isUploadingAttachments" @change="handleFileChange" /><span class="upload-icon">↑</span><strong>{{ isUploadingAttachments ? '附件上传中…' : '选择作业附件' }}</strong><small>支持 PDF、DOCX、JPG、PNG，单个文件不超过 10 MB</small></label>
      ...
      <div class="actions"><button class="secondary-button" type="button" :disabled="isSubmitting || isUploadingAttachments" @click="page = 'detail'">取消</button><button class="primary-button" type="submit" :disabled="isSubmitting || isUploadingAttachments">{{ isSubmitting ? '提交中…' : isUploadingAttachments ? '等待附件上传' : '确认提交' }}</button></div>
    </form>
    <article v-else class="panel result">
      ...
      <div v-if="displayedSubmission?.attachments.length" class="attachments"><div class="section-title"><span>本次提交附件</span><small>{{ displayedSubmission.attachments.length }} 个文件</small></div><div v-for="file in displayedSubmission.attachments" :key="file.id" class="attachment-row"><span class="file-icon">{{ file.originalName.split('.').at(-1)?.toUpperCase() ?? '文件' }}</span><strong>{{ file.originalName }}</strong><small>{{ Math.ceil(file.byteSize / 1024) }} KB</small><button class="download-button" type="button" :disabled="downloadingFileId !== null" @click="downloadAttachment(file)">{{ downloadingFileId === file.id ? '下载中…' : '下载' }}</button></div></div>
      <details v-if="submissionHistory.length > 1">
        ...
        <div v-for="item in submissionHistory" :key="item.id" class="history-item">
          <p class="history-head">第 {{ item.attempt }} 次 · {{ getSubmissionStatusLabel(item.status) }} · {{ formatDateTime(item.submittedAt) }}</p>
          <p v-if="item.content" class="history-content">正文：{{ item.content }}</p>
          <p v-if="item.score != null" class="history-score">得分：{{ item.score }}</p>
          <p v-if="item.teacherComment" class="history-comment">老师评语：{{ item.teacherComment }}</p>
        </div>
      </details>
...
<style scoped>
.assignment-workspace { max-width: 960px; margin: 0 auto; }.back-button, .secondary-button, .primary-button { border: 0; border-radius: 10px; padding: 11px 17px; font-weight: 800; cursor: pointer; transition: transform .2s, box-shadow .2s; }.back-button, .secondary-button { color: #536079; background: #eef0f7; }.primary-button { color: #fff; background: linear-gradient(135deg, #746de2, #564cc4); box-shadow: 0 8px 18px rgb(98 91 207 / 25%); }.primary-button:hover { transform: translateY(-1px); box-shadow: 0 12px 22px rgb(98 91 207 / 30%); }.primary-button:disabled, .secondary-button:disabled { opacity: .6; cursor: not-allowed; transform: none; }.panel { margin-top: 16px; padding: 32px; border: 1px solid #e4e7ef; border-radius: 22px; background: #fff; box-shadow: 0 14px 38px rgb(38 51 75 / 10%); }.detail-hero, .submit-heading { display: flex; justify-content: space-between; gap: 24px; align-items: flex-start; }.eyebrow { margin: 0; color: #6c64d7; font-size: 12px; font-weight: 850; letter-spacing: .08em; }h1 { margin: 9px 0 12px; color: #263247; font-size: clamp(27px, 4vw, 36px); }.description, .deadline { color: #68748a; line-height: 1.75; }.status-pill { flex: 0 0 auto; border-radius: 999px; padding: 7px 12px; font-size: 13px; font-weight: 800; }.status-pill.open { color: #23845f; background: #e8f7f0; }.status-pill.closed { color: #b64f55; background: #ffeded; }.meta { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 24px; }.meta div { padding: 15px; border-radius: 13px; background: #f7f8fc; }.meta dt { color: #7a8496; font-size: 12px; }.meta dd { margin: 7px 0 0; color: #34425a; font-weight: 800; }.attachments { margin: 20px 0; padding: 17px; border: 1px solid #e9ebf2; border-radius: 14px; }.section-title { display: flex; justify-content: space-between; margin-bottom: 12px; font-weight: 800; }.section-title small { color: #8991a0; font-weight: 600; }.attachment-row { display: grid; grid-template-columns: auto minmax(0, 1fr) auto auto; align-items: center; gap: 10px; padding: 10px; border-radius: 10px; background: #f8f9fd; }.attachment-row small { color: #8490a3; }.file-icon { border-radius: 6px; padding: 4px 5px; color: #ce555b; background: #ffe8e8; font-size: 10px; font-weight: 900; }.download-button { border: 1px solid #d8dcef; border-radius: 8px; padding: 7px 12px; color: #564cc4; background: #fff; font-weight: 800; cursor: pointer; }.closed-message, .error-message { margin: 18px 0; color: #b54d4f; font-weight: 700; }.field { display: grid; gap: 9px; margin-top: 22px; font-weight: 800; color: #3e4960; }.field i { color: #9299a7; font-size: 12px; font-style: normal; font-weight: 500; }.field textarea { border: 1px solid #d7dce8; border-radius: 12px; padding: 14px; font: inherit; line-height: 1.6; resize: vertical; }.field textarea:focus { outline: 3px solid rgb(98 91 207 / 15%); border-color: #746de2; }.upload-zone { display: grid; place-items: center; gap: 7px; margin-top: 18px; padding: 24px; border: 2px dashed #d9d6fa; border-radius: 14px; color: #57627a; background: #fafaff; cursor: pointer; }.upload-zone input { display: none; }.upload-zone small { color: #8b94a6; }.upload-icon { display: grid; width: 34px; height: 34px; place-items: center; border-radius: 50%; color: #625bcf; background: #eeecff; font-size: 21px; }.file-name { margin: 9px 0; color: #50607c; }.actions { display: flex; justify-content: flex-end; gap: 10px; margin-top: 24px; }.result { text-align: center; }.result-check { display: grid; width: 56px; height: 56px; place-items: center; margin: 0 auto 16px; border-radius: 50%; color: #fff; background: #42aa80; font-size: 28px; font-weight: 900; }.result details { margin: 18px 0; color: #536079; text-align: left; }.history-item { padding: 12px 14px; border: 1px solid #e9ebf2; border-radius: 12px; margin-top: 10px; background: #fbfcfe; }.history-item p { margin: 4px 0; }.history-head { font-weight: 800; color: #3f4a60; }.history-content, .history-comment, .history-score { color: #68748a; font-size: 13px; line-height: 1.6; }@media (max-width: 650px) { .detail-hero, .submit-heading { flex-direction: column; }.meta { grid-template-columns: 1fr; }.panel { padding: 22px; }.attachment-row { grid-template-columns: auto minmax(0, 1fr) auto; }.attachment-row small { display: none; } }
