...
- `npm run check`：324 项有效测试通过、0 失败；2 项受限进程的回环端口测试跳过。
- `npm run test:load`：500 次、并发 50、失败 0；吞吐量 5760.16 请求/秒，平均 8.35 ms，P50 3.96 ms，P95 48.33 ms，最大 81.59 ms。
...
- HBuilderX 5.24 已识别移动工程并安装真机运行插件；测试 APK 仍需 DCloud 登录与 Android 设备完成打包安装，当前证据不写成已通过。
