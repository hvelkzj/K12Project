...
在 Windows PowerShell 验证最新 `develop` 的干净安装、全仓检查、统一启动、五服务 HTTP、六角色会话和停止后端口释放，并补齐相关文档。
...
- 执行干净安装、lint、类型检查、271 项测试、五个工作区构建和真实服务冒烟。
- 使用六个本地账号验证登录、角色、当前用户、退出和旧令牌失效。
...
- `npm run check`：通过，271 项测试通过、0 失败、0 跳过。
- 工作区：shared 10、API 51、家长端 38、学生端 67、教师端 48、后台 57。
- 五个服务：端口 3000、5173、5174、5175、5176 均返回 HTTP 200。
- 六角色：登录 200、当前用户 200、退出 204、旧令牌 401，角色全部匹配。
- 停止统一启动后五个端口全部释放。
