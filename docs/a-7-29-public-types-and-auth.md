...
> 2026-08-24 补充：本节保留 PR #4 最初的六个基准账号记录。当前已扩充为六类角色十三个双校区账号，使用时以 [本地测试账号清单](demo-accounts.md) 为准。

## 完成结论

A 的公共包、六角色 Mock 账号和认证 API 已通过 PR #4 合并到 `develop`。四个前端的精确共享包依赖已通过 PR #7 合并；B 和 E 随后通过 PR #6、PR #5 接入真实认证。
...
- PR #5、#6、#7 合并整改后，全仓 `npm run check` 通过，共发现 72 项测试（71 项通过、1 项因上述沙箱限制跳过），六个工作区类型检查和构建均成功。
- E 已在 Windows 完成 `npm ci`、`npm run check`、`npm run dev:api` 以及登录、当前用户、退出和退出后失效验证。
