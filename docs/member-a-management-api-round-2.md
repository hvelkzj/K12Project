...
- 复核 PR #20 后发现服务端仍允许把已批准请假登记为非 `LEAVE` 状态。
- 服务端现在拒绝已批准请假的 `PRESENT`、`LATE` 和 `ABSENT` 签到。
- 请假批准前已经存在签到记录时，审批成功会把该记录同步纠正为 `LEAVE`。
- 新增独立仓库测试，覆盖审批前签到纠正、错误状态 `422` 和合法 `LEAVE` 写入。
