...
- `0.1.2` 已完成免费云打包，文件大小为 15,077,556 字节；压缩结构检查无错误。
- 内置 `manifest.json` 显示版本 `0.1.2`、版本号 `102`，并包含 Camera 模块；安装包同时包含 58 项相机/相册相关资源。
- `jarsigner -verify` 验证通过。测试包使用云端自签名证书且没有时间戳，验证工具会给出预期警告；证书与密钥未提交仓库。
- `0.1.2` APK SHA-256：`482f70027dbcc7d4f7bc3b9f053064a9b973109caa40fc434496f3c43c010099`。
- 新包已整理为桌面 `K12学习空间-Android-0.1.2.apk`；真实相机启动、授权和提交仍需在安装新包后完成一次设备复验。
...
2. 安装桌面最新版本 `K12学习空间-Android-0.1.2.apk`，不要继续使用缺少 Camera 模块的旧包。
